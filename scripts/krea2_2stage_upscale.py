...
class KreaTwoStageUpscale(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row():
            tile_overlap = gr.Slider(label="Tile Overlap", minimum=0, maximum=1024, step=16, value=96, elem_id=self.elem_id("tile_overlap"))
            tile_batch_size = gr.Slider(label="Tile Batch Size", minimum=1, maximum=8, step=1, value=1, elem_id=self.elem_id("tile_batch_size"))

        save_stage1 = gr.Checkbox(label="Save Stage 1 output", value=True, elem_id=self.elem_id("save_stage1"))
    ...
    @staticmethod
    def _enable_multidiffusion(p, method: str, tile_width: int, tile_height: int, tile_overlap: int, tile_batch_size: int):
        if p.scripts is None or p.script_args is None:
            raise RuntimeError("Krea2 2-Stage Upscale requires img2img script context.")

        multidiffusion = next((script for script in p.scripts.alwayson_scripts if script.title() == "MultiDiffusion Integrated"), None)
        if multidiffusion is None:
            raise RuntimeError("MultiDiffusion Integrated script is not loaded.")

        replacement = [True, method, int(tile_width), int(tile_height), int(tile_overlap), int(tile_batch_size)]
    ...
    def _validate_run(p, first_pass_denoise: float, final_denoise: float):
        if p.batch_size != 1 or p.n_iter != 1:
            raise ValueError("Krea2 2-Stage Upscale supports Batch Count 1 and Batch Size 1 only.")
        ...
        if p.image_mask is not None or p.latent_mask is not None:
            raise ValueError("Krea2 2-Stage Upscale supports normal img2img only; inpaint masks are not supported.")
