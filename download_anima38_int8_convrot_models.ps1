...
$Revision = "dd05532037130bebe4d94f0d559b968c14ed1279"
$SourceDirectory = Join-Path $PSScriptRoot "tmp\anima38-conversion"
$SourcePath = Join-Path $SourceDirectory "Anima-3.8B.safetensors"
$OutputPath = Join-Path $PSScriptRoot "models\Stable-diffusion\Anima-3.8B-int8-convrot.safetensors"
...
$ConvertedBytes = [Int64]4238326342
...
$Downloads = @(
    ...
    @{
        Label = "Anima 3.8B progressive Qwen3.5 adapter"
        Url = "https://huggingface.co/$Repository/resolve/$Revision/text_encoders/Anima-3.8B-expanded_adapter.safetensors"
        RelativePath = "models\text_encoder\Anima-3.8B-expanded_adapter.safetensors"
        Bytes = [Int64]88131712
        Sha256 = "f9851ac4668ce069f7be7cf99755335c98879b463f3d486aaa731083978f0d71"
        Markers = @("anima_progressive_qwen35_cross_adapter_v1", "semantic_attentions.0.q_proj.weight")
        Source = "https://huggingface.co/$Repository/tree/$Revision/text_encoders"
...
$SourceDownload = @{
    Label = "Anima 3.8B BF16 conversion source"
    Url = "https://huggingface.co/$Repository/resolve/$Revision/difussion_models/Anima-3.8B.safetensors"
    Bytes = [Int64]7504189974
    Sha256 = "1432c925752447df86da7b277e3797f077d358bc24e3950685b13cc0e465c7d5"
    Markers = @('"new_block_count":"52"', "net.blocks.51.mlp.layer2.weight")
...
function Assert-AnimaConvRotCheckpoint {
    ...
    if ($record -notmatch '^([0-9a-f]{64})  Anima-3\.8B-int8-convrot\.safetensors$') {
...
Write-Host "This setup installs Anima 3.8B with an INT8 ConvRot DiT and the released mixed-FP8 Qwen3.5 4B encoder."
Write-Host "The BF16 DiT is staged only for conversion and removed after success unless -KeepSource is used."
...
if (Test-Path -LiteralPath $OutputPath -PathType Leaf) {
    ...
    Write-Host "Existing Anima 3.8B INT8 ConvRot checkpoint is valid. Skipping conversion."
}
else {
    Download-ModelFile `
        -CurlPath $curlPath `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Anima 3.8B INT8 ConvRot conversion failed. The BF16 source was kept for retry."
    }
    Assert-AnimaConvRotCheckpoint -Path $OutputPath
}
...
Write-Host ""
Write-Host "Anima 3.8B files are ready."
Write-Host "Preset: anima"
Write-Host "Checkpoint: Anima-3.8B-int8-convrot.safetensors"
Write-Host "VAE / Text Encoder: qwen_image_vae.safetensors, qwen_3_06b_base.safetensors"
Write-Host "Anima 3.8B (Qwen3.5): enabled, adapter strength 1.0"
