...
parser.add_argument("--aikimi-remote", action="store_true", help="explicitly opt in to authenticated non-loopback/share/ngrok exposure")
...
parser.add_argument("--gradio-allowed-path", action="append", help="allow an additional path only when it is inside an Aikimi-managed output or temporary directory", default=[])
...
parser.add_argument("--api-auth-path", type=normalized_filepath, help="read API Basic-auth credentials from a local file (recommended for remote mode)", default=None)
