...
from modules_forge.krea2_highres import uses_prompt_only_conditioning_cache
...
def _scale_inpaint_crop_region(
    crop_region: tuple[int, int, int, int] | None,
    scale: float,
) -> tuple[int, int, int, int] | None:
    if crop_region is None or scale == 1.0:
        return crop_region

    return tuple(round(coordinate * scale) for coordinate in crop_region)
...
class StableDiffusionProcessing:
    ...
    def cached_params(self, required_prompts: list[str], steps: int, extra_network_data: dict, hires_steps: int | None):
        ...
        prompt_only_conditioning = uses_prompt_only_conditioning_cache(shared.sd_model)
        init_latent_hash = None
        if not prompt_only_conditioning and isinstance(
            self, StableDiffusionProcessingImg2Img
        ):
            init_latent_hash = hash_tensor(self.init_latent)
        return (
            required_prompts,
            steps,
            hires_steps,
            extra_network_data,
            None if prompt_only_conditioning else self.width,
            None if prompt_only_conditioning else self.height,
...
def _capture_override_settings(overrides: dict[str, Any]) -> dict[str, Any]:
    """Capture effective values, including defaults omitted from config.json."""
    return {
        key: opts.data[key] if key in opts.data else opts.get_default(key)
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    try:
        ...
        if p.scripts is not None:
            p.scripts.on_process_cleanup(p)
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        ...
        imgs = []
        base_overlay_mask = self.mask_for_overlay
        final_overlay_mask = base_overlay_mask

        for img in self.init_images:
            local_crop_region = crop_region
            local_overlay_mask = base_overlay_mask
            ...
            if image_mask is not None:
                ...
                if local_crop_region is not None and pre_scale != 1.0:
                    local_crop_region = _scale_inpaint_crop_region(local_crop_region, pre_scale)
                    x1, y1, x2, y2 = local_crop_region
                    self.paste_to = (x1, y1, x2 - x1, y2 - y1)

                precise_overlay_mask = orig_mask
                if pre_scale != 1.0 and opts.img2img_inpaint_precise_mask:
                    h, w = orig_mask.shape
                    precise_overlay_mask = cv2.resize(
                        orig_mask,
                        (round(w * pre_scale), round(h * pre_scale)),
                        interpolation=cv2.INTER_LINEAR,
                    )

                if local_overlay_mask.size != image.size:
                    local_overlay_mask = images.resize_image(
                        self.resize_mode,
                        local_overlay_mask,
                        image.width,
                        image.height,
                    )
                final_overlay_mask = local_overlay_mask

                if opts.img2img_inpaint_precise_mask:
                    self.overlay_images.append((image.copy(), precise_overlay_mask))
                else:
                    if local_crop_region is not None:
                        overlay_image = image
                        overlay_image.info[_INPAINT_FULL_RES_OVERLAY_MASK] = local_overlay_mask.crop(local_crop_region).convert("L")
                        self.overlay_images.append(overlay_image)
                    else:
                        image_masked = Image.new("RGBa", (image.width, image.height))
                        image_masked.paste(image.convert("RGBA").convert("RGBa"), mask=ImageOps.invert(local_overlay_mask.convert("L")))
                        self.overlay_images.append(image_masked.convert("RGBA"))

            if local_crop_region is not None:  # Only masked
                image = image.crop(local_crop_region)
        ...
        self.mask_for_overlay = final_overlay_mask
