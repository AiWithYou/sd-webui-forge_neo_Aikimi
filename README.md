...
キャラクターは設定資料に近い約2頭身で、`idle`、モデル読込、生成、完了、Queue、警告、エラー、VRAM不足、更新の各状態に専用ポーズを備えています。APNGでidleの穏やかな呼吸、作業中の淡い発光、完了時の短い喜びを表現し、OSでモーション低減を有効にしている場合は同じ状態の静止WebPへ自動的に切り替わります。
