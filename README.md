...
Standard Anima LoRA: None、または1件目のLoRA
Standard LoRA strength: 1.0
Additional Standard Anima LoRAs: 必要な場合だけ展開
Standard Anima LoRA 2-4: 追加するLoRAと個別のstrength
Low VRAM encoder offload: off
```

`Standard Anima LoRA`に表示されるのは、`models/Lora`で検出した完全な28層・40層・52層のsafetensors LoRAだけです。28層版と40層版は52層へ自動展開され、`Additional Standard Anima LoRAs`を開けば最大4件をそれぞれ異なるstrengthで設定できます。起動後にLoRAを追加した場合は`Refresh standard Anima LoRAs`を押してください。5件以上を組み合わせる場合や通常の操作方法を使う場合は、LoRAタブまたは`<lora:name:weight>`タグを利用します。
